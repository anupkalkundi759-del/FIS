def run_engine(conn, cur):
    ...
    start_sql = """
        SELECT h.house_no, MIN(t.timestamp) AS actual_start
        FROM houses h
        JOIN products p ON h.house_id=p.house_id
    ...
    start_df = pd.DataFrame(cur.fetchall(), columns=["house","actual_start"])
    ...
    if selected_house != "ALL":
        finish_sql += " AND h.house_no=%s" if "WHERE" in finish_sql else " WHERE h.house_no=%s"
        params += (selected_house,)
