def run_engine(conn, cur):
    ...
    with left_col:
        ...
        if st.button("Save BAC"):
            if project_id is not None and unit_id is not None:
                ...
                st.warning("Select Project + Unit")
    ...
    with right_col:
        ...
        cur.execute("""
            ...
            ORDER BY baseline_start_date DESC
            LIMIT 1
        """)
        ...
        if st.button("Save Quarter Plan"):
            cur.execute("DELETE FROM evm_quarter_plan")
    ...
    if selected_house != "ALL":
        live_sql += " AND h.house_no=%s" if "WHERE" in live_sql else " WHERE h.house_no=%s"
    ...
    live_df = live_df.drop_duplicates(
        subset=["house_id", "product_instance_id"],
        keep="last"
    )

    start_sql = """
        SELECT h.house_id, MIN(t.timestamp) AS actual_start
        FROM houses h
    ...
    if selected_house != "ALL":
        start_sql += " AND h.house_no=%s" if "WHERE" in start_sql else " WHERE h.house_no=%s"
        params += (selected_house,)

    start_sql += " GROUP BY h.house_id"

    cur.execute(start_sql, params)

    start_df = pd.DataFrame(cur.fetchall(), columns=["house_id", "actual_start"])
    ...
    start_map = dict(zip(start_df["house_id"], start_df["actual_start"]))
    ...
    finish_sql = """
        ...
        SELECT
            ...
            COUNT(
                DISTINCT CASE
                    WHEN stage_name='Dispatch'
                    AND rn=1
                    THEN product_instance_id
                END
            ) dispatched_products,
    ...
    loop_houses = live_df["house_id"].unique()

    for house_id_loop in loop_houses:

        sub = live_df[
            live_df["house_id"] == house_id_loop
        ].copy()

        display_house = sub["house"].iloc[0]

        total_products = sub["product_instance_id"].nunique()

        actual_prog = round(sub["earned_pct"].mean(), 2) if total_products > 0 else 0

        if house_id_loop in start_map:
            start_dt = start_map[house_id_loop]
            planned_finish = (start_dt + timedelta(days=total_duration)).date()
        else:
            start_dt = None
            planned_finish = "Not Started"

        dispatch_info = finish_map.get(house_id_loop, {})
        ...
        if total_products == 0:
            ...
            remaining_products = max(0, total_products - dispatched)

            actual_stage_output = throughput_map.get(dominant_stage, 0)

            if actual_stage_output > 0:
                ettc_days = int(round(remaining_products / actual_stage_output))
            else:
                ettc_days = total_duration
