def run_engine(conn, cur):
    ...
    with left_col:
        ...
        if project_id is not None and unit_id is not None:
            ...
            cur.execute(
                "SELECT bac_amount FROM project_evm_baseline WHERE project_id=%s AND unit_id=%s",
                (project_id, unit_id)
            )
    ...
    with right_col:
        ...
        elif project_id is not None and unit_id is None:
        ...
        quarter_base_date = st.date_input(
            "Quarter Baseline Start Date",
            value=ex_qdate,
            key="q_base"
        )

        target_days_input = st.number_input(
            "Core Production Days",
            min_value=0,
            value=ex_tdays,
            step=1,
            key="q_target"
        ...
        buffer_days_input = st.number_input(
            "Buffer / Float Days",
            min_value=0,
            value=ex_bdays,
            step=1,
    ...
    activity_df["earned_pct"] = round(
        (activity_df["cum_days"] / total_duration) * 100,
        2
    )
    ...
    if unit_id is not None:
        ...
        cur.execute(
            "SELECT COUNT(*) FROM products p JOIN houses h ON p.house_id=h.house_id WHERE h.unit_id=%s",
            (unit_id,)
        )
    ...
    live_df = live_df.drop_duplicates(
        subset=["house", "product_instance_id"]
    )
    ...
    start_sql = """
        SELECT h.house_no,
               MIN(t.timestamp) AS actual_start
        FROM houses h
        JOIN products p ON h.house_id=p.house_id
    ...
    start_df = pd.DataFrame(
        cur.fetchall(),
        columns=["house", "actual_start"]
    )

    start_df["actual_start"] = pd.to_datetime(
        start_df["actual_start"],
        utc=True,
        errors="coerce"
    ).dt.tz_convert(tz)

    start_map = dict(zip(start_df["house"], start_df["actual_start"]))
    ...
    finish_sql += """
        ...
        SELECT house_no,
               COUNT(product_instance_id) total_products,
               COUNT(CASE WHEN stage_name='Dispatch' AND rn=1 THEN 1 END) dispatched_products,
    ...
    live_df["earned_pct"] = live_df["stage"].map(
        lambda x: earned_map.get(x, 0.0)
    )

    project_actual_progress = round(
        live_df["earned_pct"].mean(),
        2
    )
    ...
    if pvrow and pvrow[0] and pvrow[1] is not None and pvrow[2] is not None:
        ...
        elapsed_batch = max(
            0,
            (today - base_start).days
        )

        project_planned_progress = min(
            100,
            round((elapsed_batch / planned_horizon) * 100, 2)
        ) if planned_horizon > 0 else 0
    ...
    if project_id is not None and unit_id is not None:
        ...
        cur.execute(
            "SELECT COALESCE(SUM(actual_cost),0) FROM evm_cost_log WHERE project_id=%s AND unit_id=%s",
            (project_id, unit_id)
        )
        ...
        cur.execute(
            "SELECT COALESCE(SUM(bac_amount),0) FROM project_evm_baseline WHERE project_id=%s",
            (project_id,)
        )
        ...
        cur.execute(
            "SELECT COALESCE(SUM(actual_cost),0) FROM evm_cost_log WHERE project_id=%s",
            (project_id,)
        )
    ...
    cap_map = {x[0]: x[1] for x in cur.fetchall()}
    ...
    throughput_map = {
        x[0]: round(float(x[1]), 2) if x[1] else 0
    ...
    if selected_house != "ALL":
        loop_houses = [selected_house]
    else:
        loop_houses = sorted(
            live_df["house"].dropna().unique().tolist()
        )
    ...
    for house in loop_houses:
        ...
        if total_products == 0:
            ...
            delay_days = max(
                0,
                (predicted_finish - planned_finish).days
            ) if planned_finish != "Not Started" else 0
            ...
            if dominant_stage in bottleneck_df["Stage"].values:
                ...
                q_penalty = float(
                    bottleneck_df[
                        bottleneck_df["Stage"] == dominant_stage
                    ]["Queue Load Days"].iloc[0]
                )

            spi_penalty = (1 / SPI) if SPI > 0 and SPI < 1 else 1

            ettc_days = int(round(
                (base_remaining_days * spi_penalty) + q_penalty
            ))
    ...
    if SPI < 1:
        warning_rows.append({
            "Alert": f"Project SPI below 1 ({SPI}) - Schedule Slipping"
        })

    if CPI > 0 and CPI < 1:
        warning_rows.append({
            "Alert": f"Project CPI below 1 ({CPI}) - Cost Burn Higher Than Earned"
        })

    if bottleneck_stage:
        warning_rows.append({
            "Alert": f"Critical Bottleneck Detected at {bottleneck_stage}"
        })

    for _, rr in house_df[house_df["Health"] == "No Movement"].iterrows():
        warning_rows.append({
            "Alert": f"House {rr['House']} has no recent movement"
        })

    for _, rr in house_df[house_df["Health"] == "Critical Delay"].iterrows():
        warning_rows.append({
            "Alert": f"House {rr['House']} critically delayed by {rr['Delay Days']} days"
        })
