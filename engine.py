def run_engine(conn, cur):
    ...
    # =========================================================
    # HOUSE MASTER FETCH FOR FULL VISIBILITY + SLA DROPDOWN
    # =========================================================
    ...
    house_dict = {x[1]: x[0] for x in all_houses}
    full_house_list = [x[1] for x in all_houses]

    with r2c3:
        sla_house = st.selectbox("SLA Monitor House", full_house_list, key="sla_house")
    ...
    # =========================================================
    total_houses = len(full_house_list)
    ...
    start_map = dict(zip(start_df["house"], start_df["measure_start"])) if not start_df.empty else {}
    ...
    finish_map = finish_df.set_index("house").to_dict("index") if not finish_df.empty else {}
    ...
    SV = EV - PV
    CV = EV - AC
    SPI = round(project_actual_progress / 100, 2)
    ...
    throughput_map = {x[0]: x[1] for x in thr_rows}

    bottleneck_stage = None
    highest_pressure = -1

    for stage in activity_df["stage"].tolist():
        current_wip = int((live_df["stage"] == stage).sum()) if not live_df.empty else 0
        cap_day = cap_map.get(stage, 1)
        pressure = round((current_wip / cap_day) * 100, 1) if cap_day > 0 else 0

        if pressure > highest_pressure and current_wip >= 3:
    ...
    for house in full_house_list:
        ...
        actual_prog = round(sub["earned_pct"].mean(), 2) if not sub.empty else 0
        ...
        if total_products == 0:
            ...
            spi_penalty = (1 / SPI) if SPI > 0 else 1
            ettc_days = int(round(base_remaining_days * spi_penalty))

            predicted_finish_dt = today + timedelta(days=ettc_days)
            predicted_finish = predicted_finish_dt.date()
            actual_finish = "Not Finished"
            delay_days = ettc_days

            if ettc_days > total_duration:
                delayed_houses += 1

            last_move = sub["timestamp"].max()
            stagnant_days = (today - last_move).days if pd.notna(last_move) else 0

            if stagnant_days >= 3:
                health = "No Movement"
            elif dominant_stage == bottleneck_stage:
                health = "Bottleneck Hit"
            elif ettc_days > total_duration:
                health = "Delayed"
            else:
                health = "On Track"
    ...
    if SPI < 1:
        warning_rows.append({"Alert": f"Project SPI below 1 ({SPI}) - Schedule Slow"})

    if CPI > 0 and CPI < 1:
        warning_rows.append({"Alert": f"Project CPI below 1 ({CPI}) - Cost Burn High"})

    if bottleneck_stage:
        warning_rows.append({"Alert": f"Critical Bottleneck at {bottleneck_stage}"})

    stagnant = house_df[house_df["Health"] == "No Movement"]
    for _, rr in stagnant.iterrows():
        warning_rows.append({"Alert": f"House {rr['House']} no recent movement"})
    ...
    st.subheader("📈 EVM Executive Control Board")
    ...
    st.dataframe(house_df, use_container_width=True, height=500)

    st.subheader("🎯 SLA Priority Monitor")
    if not sla_df.empty:
        st.dataframe(
            sla_df.sort_values("Priority", ascending=False),
    ...
    st.subheader("🔄 Factory Flow Throughput Monitor")
    st.dataframe(flow_df, use_container_width=True, height=260)
